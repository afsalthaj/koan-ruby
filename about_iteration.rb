...
class AboutIteration < Neo::Koan
  ...
  def test_if_all_methods_are_symbols
    array = Array.new
    methods = [].methods
    i = 0
    while i < methods.size
      if methods[i].is_a?(String)
      array <<  methods[i]
      end
      i += 1
    end
  ...
  def test_inject_will_blow_your_mind
    ...
    # The inject method is a method wrongly named (it is similar to fold in scala)
    # which is used to reduce the collection to a value, whose default value defines its type.
  end
